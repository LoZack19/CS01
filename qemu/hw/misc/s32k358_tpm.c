...
static UINT16 read_be16(Fifo8 *fifo) {
    return ((UINT16)fifo8_pop(fifo) << 8) | (UINT16)fifo8_pop(fifo);
}

static UINT32 read_be32(Fifo8 *fifo) {
...
static void write_be16(Fifo8 *fifo, UINT16 value) {
    fifo8_push(fifo, (value >> 8) & 0xFF);
    fifo8_push(fifo, value & 0xFF);
}

static void write_be32(Fifo8 *fifo, UINT32 value) {
...
static void tpm_cmd_header_unmarshal(Fifo8 *fifo, tpm_cmd_header_t *header) {
...
static void tpm_rsp_header_marshal(Fifo8 *fifo, const tpm_rsp_header_t *header) {

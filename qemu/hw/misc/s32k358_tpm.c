...
static void s32k358_tpm_process_input(S32k358TPMState *s) {
    ...
    if (fifo8_num_used(&s->infifo) < sizeof(tpm_cmd_header_t)) {
        qemu_log_mask(LOG_GUEST_ERROR,
                      "(ERROR) TPM: Insufficient fifo DATA\n ");
        return;
    }
    ...
    if (cmd_header.tag != TPM_ST_NO_SESSIONS &&
        ...
        qemu_log_mask(
            LOG_GUEST_ERROR,
            "(ERROR) TPM: Command header tag is not valid. Received 0x%04X\n",
            cmd_header.tag);
    ...
    if (!tpm_command_allowed_in_current_mode(s, cmd_header.commandCode, &rc)) {
    ...
    if (fifo8_num_used(&s->infifo) <
        ...
        qemu_log_mask(LOG_GUEST_ERROR, "(ERROR) TPM: FIFO does not have enough "
                                       "data wrt the specified command size\n");
        qemu_log_mask(LOG_GUEST_ERROR,
                      "(INFO) TPM: Command size: %u, Header size: %zu\n",
                      cmd_header.commandSize, sizeof(tpm_cmd_header_t));
        qemu_log_mask(
            LOG_GUEST_ERROR,
            "(INFO) TPM: FIFO used size: %u, FIFO available size: %u\n",
    ...
    qemu_log_mask(LOG_GUEST_ERROR,
                  "(INFO) TPM: Processing command 0x%08X (size=%u, fifo=%u)\n",
                  cmd_header.commandCode, cmd_header.commandSize,
                  fifo8_num_used(&s->infifo));

    switch (cmd_header.commandCode) {
    case TPM_CC_Startup: {
        Startup_In startup_in;

        if (cmd_header.commandSize !=
            sizeof(tpm_cmd_header_t) + sizeof(startup_in)) {
            tpm_send_error_response(s, TPM_RC_COMMAND_SIZE);
            return;
        }

        UNMARSHAL(&startup_in, &s->infifo);
        rc = TPM2_Startup_SM(s, &startup_in);
    ...
    case TPM_CC_Shutdown: {
        ...
        if (cmd_header.commandSize !=
            sizeof(tpm_cmd_header_t) + sizeof(shutdown_in)) {
            tpm_send_error_response(s, TPM_RC_COMMAND_SIZE);
            return;
        }

        UNMARSHAL(&shutdown_in, &s->infifo);
        rc = TPM2_Shutdown_SM(s, &shutdown_in);
        tpm_send_response(s, rc, NULL, 0);
        return;
    }

    case TPM_CC_SelfTest: {
        SelfTest_In self_test_in;

        if (cmd_header.commandSize !=
            sizeof(tpm_cmd_header_t) + sizeof(self_test_in)) {
            tpm_send_error_response(s, TPM_RC_COMMAND_SIZE);
            return;
        }

        UNMARSHAL(&self_test_in, &s->infifo);
        rc = TPM2_SelfTest_SM(s, &self_test_in);
        tpm_send_response(s, rc, NULL, 0);
        return;
    }

    case TPM_CC_GetTestResult: {
        GetTestResult_Out get_test_result_out;
        ...
        rc = TPM2_GetTestResult_SM(s, &get_test_result_out);
        tpm_send_response(s, rc, &get_test_result_out,
                          sizeof(get_test_result_out));
        return;
    }

    case TPM_CC_GetCapability: {
        GetCapability_In get_capability_in;
        GetCapability_Out get_capability_out;

        if (cmd_header.commandSize !=
            sizeof(tpm_cmd_header_t) + sizeof(get_capability_in)) {
            tpm_send_error_response(s, TPM_RC_COMMAND_SIZE);
            return;
        }

        UNMARSHAL(&get_capability_in, &s->infifo);
        rc = TPM2_GetCapability_SM(s, &get_capability_in, &get_capability_out);
        tpm_send_response(s, rc, &get_capability_out,
                          sizeof(get_capability_out));
        return;
    }
    ...
    case TPM_CC_FieldUpgradeStart:
        ...
        rc = TPM2_FieldUpgradeStart_SM(s);
        tpm_send_response(s, rc, NULL, 0);
        return;

    case TPM_CC_FieldUpgradeData: {
        FieldUpgradeData_In field_upgrade_data_in;

        if (cmd_header.commandSize !=
            sizeof(tpm_cmd_header_t) + sizeof(field_upgrade_data_in)) {
            tpm_send_error_response(s, TPM_RC_COMMAND_SIZE);
            return;
        }

        UNMARSHAL(&field_upgrade_data_in, &s->infifo);
        rc = TPM2_FieldUpgradeData_SM(s, &field_upgrade_data_in);
        tpm_send_response(s, rc, NULL, 0);
        return;
    }
    ...
    case TPM_CC_GetRandom: {
        ...
        if (cmd_header.commandSize !=
            sizeof(tpm_cmd_header_t) + sizeof(get_random_in)) {
            tpm_send_error_response(s, TPM_RC_COMMAND_SIZE);
            return;
        }
    ...
    case TPM_CC_NV_DefineSpace: {
        ...
        // Check if the command size is coherent with the expected size
        if (cmd_header.commandSize !=
            sizeof(tpm_cmd_header_t) + sizeof(nv_define_space_in)) {
            tpm_send_error_response(s, TPM_RC_COMMAND_SIZE);
            return;
        }
    ...
    case TPM_CC_NV_Write: {
        qemu_log_mask(LOG_GUEST_ERROR,
                      "(INFO) TPM: Processing NV_Write command\n");
        NV_Write_In nv_write_in;

        // Check if the command size is coherent with the expected size
        if (cmd_header.commandSize !=
            sizeof(tpm_cmd_header_t) + sizeof(nv_write_in)) {
            qemu_log_mask(
                LOG_GUEST_ERROR,
                "(ERROR) TPM: NV_Write size mismatch: expected %zu, got %u\n",
                sizeof(tpm_cmd_header_t) + sizeof(nv_write_in),
                cmd_header.commandSize);
            tpm_send_error_response(s, TPM_RC_COMMAND_SIZE);
            return;
        }
        ...
        qemu_log_mask(LOG_GUEST_ERROR,
                      "(INFO) TPM: NV_Write returned rc=0x%X\n", rc);
    ...
    case TPM_CC_NV_Read: {
        qemu_log_mask(LOG_GUEST_ERROR,
                      "(INFO) TPM: Processing NV_Read command\n");
        NV_Read_In nv_read_in;
        NV_Read_Out nv_read_out;

        // Check if the command size is coherent with the expected size
        if (cmd_header.commandSize !=
            sizeof(tpm_cmd_header_t) + sizeof(nv_read_in)) {
            tpm_send_error_response(s, TPM_RC_COMMAND_SIZE);
            return;
        }
    ...
    case TPM_CC_Sign: {
        ...
        if (cmd_header.commandSize !=
            sizeof(tpm_cmd_header_t) + sizeof(sign_in)) {
            tpm_send_error_response(s, TPM_RC_COMMAND_SIZE);
            return;
        }
    ...
    case TPM_CC_VerifySignature: {
        ...
        if (cmd_header.commandSize !=
            sizeof(tpm_cmd_header_t) + sizeof(verify_in)) {
            tpm_send_error_response(s, TPM_RC_COMMAND_SIZE);
            return;
        }
    ...
    case TPM_CC_Hash: {
        ...
        if (cmd_header.commandSize !=
            sizeof(tpm_cmd_header_t) + sizeof(hash_in)) {
            tpm_send_error_response(s, TPM_RC_COMMAND_SIZE);
            return;
        }
    ...
    case TPM_CC_EncryptDecrypt2: {
        ...
        if (cmd_header.commandSize !=
            sizeof(tpm_cmd_header_t) + sizeof(encrypt_in)) {
            tpm_send_error_response(s, TPM_RC_COMMAND_SIZE);
            return;
        }
    ...
    case TPM_CC_RSA_Encrypt: {
        ...
        if (cmd_header.commandSize !=
            sizeof(tpm_cmd_header_t) + sizeof(rsa_encrypt_in)) {
            tpm_send_error_response(s, TPM_RC_COMMAND_SIZE);
            return;
        }
    ...
    case TPM_CC_RSA_Decrypt: {
        ...
        if (cmd_header.commandSize !=
            sizeof(tpm_cmd_header_t) + sizeof(rsa_decrypt_in)) {
            tpm_send_error_response(s, TPM_RC_COMMAND_SIZE);
            return;
        }
    ...
    case TPM_CC_CreatePrimary: {
        ...
        /* Send response */
        if (hasAuth) {
            tpm_rsp_header_t rsp = {.tag = TPM_ST_SESSIONS, .responseCode = rc};
            ...
            if (rc == TPM_RC_SUCCESS) {
                ...
                rsp.responseSize =
                    sizeof(rsp) + sizeof(TPMS_AUTH_RESPONSE_AREA);
            }
            ...
            tpm_send_response(s, rc, &create_primary_out,
                              sizeof(create_primary_out));
        }
    ...
    case TPM_CC_Create: {
        ...
        if (cmd_header.commandSize !=
            sizeof(tpm_cmd_header_t) + sizeof(create_in)) {
            tpm_send_error_response(s, TPM_RC_COMMAND_SIZE);
            return;
        }
    ...
    case TPM_CC_Load: {
        ...
        if (cmd_header.commandSize !=
            sizeof(tpm_cmd_header_t) + sizeof(load_in)) {
            tpm_send_error_response(s, TPM_RC_COMMAND_SIZE);
            return;
        }
    ...
    case TPM_CC_ReadPublic: {
        ...
        if (cmd_header.commandSize !=
            sizeof(tpm_cmd_header_t) + sizeof(read_public_in)) {
            tpm_send_error_response(s, TPM_RC_COMMAND_SIZE);
            return;
        }
...
static uint64_t s32k358_tpm_read(void *opaque, hwaddr offset, unsigned size) {
    ...
    case A_TPM_DATA_FIFO:
        if (fifo8_is_empty(&s->outfifo)) {
            qemu_log_mask(LOG_GUEST_ERROR,
                          "%s: Output FIFO is empty, cannot read\n", __func__);
            return 0;
        }
        {
            ...
            s->tpm_sts |=
                (fifo8_num_used(&s->outfifo) << R_TPM_STS_burstCount_SHIFT) &
...
static void s32k358_tpm_write(void *opaque, hwaddr offset, uint64_t value,
    ...
    case A_TPM_ACCESS:
        ...
        if (value & R_TPM_ACCESS_requestUse_MASK) {
            ...
            qemu_log_mask(LOG_GUEST_ERROR,
                          "%s: Locality requested, granting access\n",
                          __func__);
    ...
    case A_TPM_DATA_FIFO:
        if (fifo8_is_full(&s->infifo)) {
            qemu_log_mask(LOG_GUEST_ERROR,
                          "%s: Input FIFO is full, cannot write 0x%" PRIx64
                          "\n",
                          __func__, value);
            return;
        }
        ...
        if (s->tpm_state == TPM_S_RECV) {
            ...
            s->tpm_sts &= ~R_TPM_STS_burstCount_MASK;
            s->tpm_sts |=
                (fifo8_num_free(&s->infifo) << R_TPM_STS_burstCount_SHIFT) &
                R_TPM_STS_burstCount_MASK;
            ...
            qemu_log_mask(LOG_GUEST_ERROR,
                          "(ERROR) TPM: Data write ignored, state=%d\n",
                          s->tpm_state);
        }
        break;
    case A_TPM_STS:
        ...
        if (value & R_TPM_STS_tpmGo_MASK) {
            qemu_log_mask(
                LOG_GUEST_ERROR,
                "(INFO) TPM: tpmGo received, state=%d, fifo_used=%u\n",
                s->tpm_state, fifo8_num_used(&s->infifo));
            if (s->tpm_state == TPM_S_RECV) {
                s->tpm_state = TPM_S_EXEC;
                s32k358_tpm_process_input(s);
                // After processing, don't process commandReady in the same
                // write
                break;
            }
        }
...
static void s32k358_tpm_reset(DeviceState *d) {
    ...
    s->tpm_sts |= (TPM_STS_burstCount_RST << R_TPM_STS_burstCount_SHIFT) &
                  R_TPM_STS_burstCount_MASK;

    fifo8_reset(&s->infifo);
    fifo8_reset(&s->outfifo);

    tpm_state_machine_reset(s);
...
static void s32k358_tpm_init(Object *obj) {
    ...
    memory_region_init_io(&s->iomem, obj, &s32k358_tpm_ops, s, TYPE_S32K358_TPM,
                          S32K358_TPM_MEM_SIZE);
